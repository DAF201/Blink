    ...
    try:
        ...
        for url in urls:
            ...
            if download:
                ...
                with open(downloads_path+'/%s.png' % random_gen(), 'wb')as file:
...
def download_video():
    ...
    try:
        ...
        with open('./temp/temp.mp4', 'rb')as file:
            b64 = base64.b64encode(file.read())

        video = '''
        <video controls playsinline src='data:video/mp4;base64,%s'>
        ''' % str(b64)[2:-1]

        if download:

            with open(downloads_path+'/%s.mp4' % random_gen(), 'wb')as file:
                file.write(raw_bytes)

        return image_download_page1+video+image_download_page2
        # return send_file(desktop+r'\blink\temp\temp.mp4', as_attachment=True)
...
def download_file():
    ...
    try:
        ...
        with open(downloads_path+'/'+random_gen()+'.file', 'wb')as file:
            file.write(raw_bytes)

        return 'file downloaded successfully, check your download path'
...
def preview():
    '''manage all files in download'''

    files = os.chdir(downloads_path)
    ...
    for file in files:
        if file != 'desktop.ini':
            if file != '~$D-220 Command List-V1.1.docx':
                manage += '''
                    <p>%s</p>
                    <form action='/remove' method='post'>
                        　　　<input type='text' id='text' name='text' placeholder="enter name of file to remove">
                        　　　<button type="submit" value="submit">remove</button>
                    </form>
                ''' % file

    return download_page_part1 + manage + download_page_part2
